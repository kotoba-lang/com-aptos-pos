(ns aptos_pos.main
  "Kotodama WASM entrypoint for the Aptos_pos clean-room actor (L4) — Clojure port.
  ...
  facts (`aptos_pos.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aptos_pos")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "aptospos_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "aptospos_cus"
    :fields [:email :firstName :lastName :phone]   :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection"    :plural "collections"    :id-prefix "aptospos_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"aptos_pos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aptos_pos-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aptos_pos-compat/ready)
